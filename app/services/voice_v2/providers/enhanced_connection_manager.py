...
class IConnectionManager(ABC):
    ...
    @abstractmethod
    async def register_provider(self, provider_name: str, config: Optional[ConnectionConfig] = None) -> None:
...
class EnhancedConnectionManager(IConnectionManager):
    ...
    async def initialize(self) -> None:
        ...
        async with self._lock:
            ...
            try:
                ...
                raise ConfigurationError(f"Connection manager initialization failed: {e}")
    ...
    async def _create_provider_session(self, provider_name: str, config: ConnectionConfig) -> None:
        ...
        logger.debug(f"Provider-specific session created for: {provider_name}")
        ...
        **kwargs
        ...
        for attempt in range(config.max_retries + 1):
            try:
                ...
                logger.debug(
                    f"Request successful for {provider_name} "
                    f"(attempt {attempt + 1}, response_time: {response_time:.3f}s)"
                )
                ...
                logger.warning(
                    f"Request failed for {provider_name}: {e} "
                    f"(attempt {attempt + 1}/{config.max_retries + 1})"
                )
            ...
            if attempt < config.max_retries:
                ...
                logger.debug(f"Retrying in {delay:.3f}s...")
                ...
                if circuit_breaker.should_open(config.circuit_breaker_threshold):
                    ...
                    logger.error(
                        ...
                        f"({circuit_breaker.failure_count} consecutive failures)"
                    )
    ...
    async def health_check_all(self) -> Dict[str, ConnectionStatus]:
        ...
        logger.debug(f"Health check completed for {len(results)} providers")
        return results
        ...
        try:
            ...
            for provider_name, session in self._sessions.items():
                try:
                    ...
                    logger.error(f"Error closing session for {provider_name}: {e}")
            ...
            logger.error(f"Error during connection manager shutdown: {e}", exc_info=True)
