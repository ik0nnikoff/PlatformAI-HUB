"""
Voice_v2 Orchestrator - Main Implementation

Main VoiceServiceOrchestrator class consolidated from orchestrator/ directory.
Simplified architecture with direct implementation.
"""
...
from .exceptions import VoiceServiceError, AudioProcessingError
...
class VoiceServiceOrchestrator:
    ...
    Coordinates voice operations using provider factory pattern:
    - Enhanced Factory: Dynamic provider creation
    - Provider fallback chains: OpenAI → Google → Yandex
    - Cache and file management integration
    """
    ...
    async def _perform_health_checks(self) -> None:
        ...
        # Check STT providers
        await self._check_providers_health(self._stt_providers, "STT")

        # Check TTS providers
        await self._check_providers_health(self._tts_providers, "TTS")

    async def _check_providers_health(
        self, providers: Dict, provider_service_type: str
    ) -> None:
        """Check health of providers for a specific service type."""
        for provider_type, provider in providers.items():
            if hasattr(provider, 'health_check'):
                await self._check_single_provider_health(
                    provider, provider_type, provider_service_type
                )

    async def _check_single_provider_health(
        self, provider, provider_type, service_type: str
    ) -> None:
        ...
        try:
            ...
            if not is_healthy:
                logger.warning(
                    "%s provider %s failed health check", service_type, provider_type
                )
        except (ConnectionError, TimeoutError, OSError) as e:
            logger.warning(
                "%s provider %s health check error: %s", service_type, provider_type, e
            )

    @classmethod
    async def create_with_enhanced_factory(
        cls,
        factory_config: Dict[str, Any],  # pylint: disable=unused-argument
    ...
    async def _download_audio_from_url(self, audio_url: str) -> bytes:
        ...
        except (aiohttp.ClientError, ConnectionError, OSError) as download_error:
