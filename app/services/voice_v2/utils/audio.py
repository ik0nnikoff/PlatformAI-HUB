...
class AudioProcessor:
    ...
    def _detect_format_by_extension(filename: str) -> AudioFormat:
        ...
        extension_map = {
            ...
            '.aac': AudioFormat.M4A,
            '.m4a': AudioFormat.M4A,
            '.pcm': AudioFormat.WAV  # PCM обычно в WAV контейнере
        }
    ...
    def validate_audio(
        ...
        try:
            # Базовые проверки
            basic_validation = self._validate_basic_audio_data(audio_data)
            if not basic_validation.is_valid:
                return basic_validation

            # Проверка размера
            size_validation = self._validate_audio_size(audio_data, max_size_mb)
            if not size_validation.is_valid:
                return size_validation

            # Извлечение детальных метаданных
    ...
    def _validate_basic_audio_data(self, audio_data: bytes) -> AudioMetadata:
        """Базовая валидация аудиоданных."""
        if not audio_data:
            return AudioMetadata(
                format=AudioFormat.WAV,
                is_valid=False,
    ...
    def _validate_audio_size(self, audio_data: bytes, max_size_mb: Optional[int]) -> AudioMetadata:
        """Валидация размера аудиофайла."""
        size_bytes = len(audio_data)
    ...
    def _extract_detailed_metadata(
        self, audio_data: bytes, max_duration_seconds: Optional[int]
    ) -> AudioMetadata:
        """Извлечение детальных метаданных с проверкой длительности."""
        audio_format = self.detect_format(audio_data)
        size_bytes = len(audio_data)
        ...
        if PYDUB_AVAILABLE:
            try:
                ...
                duration_validation = self._validate_audio_duration(
    ...
    def _validate_audio_duration(
        self, duration_seconds: float, max_duration_seconds: Optional[int]
    ) -> Optional[str]:
        """Валидация длительности аудио. Возвращает None если валидно."""
