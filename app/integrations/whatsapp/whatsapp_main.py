...
from .runner_utils import WhatsAppRunner
...
async def main_async_runner(
    ...
    runner = WhatsAppRunner(agent_id, logger)
    await runner.run_integration(integration_settings)
...
if __name__ == "__main__":
    ...
    try:
        asyncio.run(main_async_runner(args.agent_id, args.integration_settings, logger_adapter))
    except KeyboardInterrupt:
        logger_adapter.info("WhatsApp bot runner main received KeyboardInterrupt. Exiting.")
    except (RuntimeError, OSError, ValueError) as e:
        logger_adapter.critical(f"Critical error in WhatsApp bot runner main: {e}", exc_info=True)
        sys.exit(1)
    finally:
