...
def auth_tool() -> str:
    ...
    return "Authorization required. Please use /login or share contact. [AUTH_REQUIRED]"
...
def get_bonus_points(state: Annotated[dict, InjectedState]) -> str:
    ...
    if user_data.get("is_authenticated"):
        ...
        if not user_phone:
            return "User phone number not found to check points."
        try:
            ...
            if response.text.strip().isdigit():
                return f"Bonus points balance: {response.text}"
            else:
    ...
    return "Authorization required to view bonus points. Please authorize first."

@tool
def get_user_info_tool(state: Annotated[dict, InjectedState]) -> str:
    """
    Gets information about the user if authenticated.

    Args:
        state (dict): The current agent state containing user_data and channel.

    Returns:
        str: User information or an authorization request.
    """
    ...
    if user_data.get("is_authenticated"):
        ...
        return f"Channel: {channel}. User information unavailable without authorization. Please authorize first."
